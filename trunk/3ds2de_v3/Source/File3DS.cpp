...
void cFile3DS::ParseFaceMaterial( FILE* fp ) 
{
    ...
    if( !stricmp( MaterialName.c_str(), "SKIN" ) ) {
		...
		{ 
			char* MatID = strrchr( MaterialName.c_str(), '.' );
			if( MatID != NULL )
			{
				while( *++MatID != 0 )
				{	
					switch( static_cast<char>(toupper(*MatID)) )
					{
						// Base types
						case 'D':  Type |= 0x01; break; // Double-sided
						case 'T':  Type |= 0x02; break; // Translucent double-sided
						case 'M':  Type |= 0x03; break; // Masked double-sided
						case '0':  Type |= 0x04; break; // Modulated double-sided
						case 'W':  Type |= 0x08; break; // Weapon marker
							
						// Modifiers
						case 'U':  Type |= 0x10; break; // Unlit
						case 'F':  Type |= 0x20; break; // Flat
						case 'E':  Type |= 0x40; break; // Environment mapped
						case 'S':  Type |= 0x80; break; // No texture smoothing
					}
				}
			}
		}
